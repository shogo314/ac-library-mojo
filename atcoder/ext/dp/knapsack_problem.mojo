...
fn knapsack_01(vw: List[(Int, Int)], weight_limit: Int) -> Int:
    ...
    if 2 ** (len(vw) // 2 + 1) < min(sum_value, weight_limit) * len(vw):
        return _knapsack_01_meet_in_the_middle(vw, weight_limit)
    elif sum_value < weight_limit:
        return _knapsack_01_value(vw, weight_limit, sum_value)
    else:
        return _knapsack_01_weight(vw, weight_limit)
...
fn _knapsack_01_meet_in_the_middle(
